...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    let remoteAccessConsumerKey = "作成した接続アプリケーションのコンシューマ鍵をここに入れてください。"
    let oauthRedirectURI = "作成した接続アプリケーションのコールバックURLをここに入れてください。"
    ...
    private func initializeAppViweState() {
        let board = UIStoryboard(name: "Storyboardのファイル名をここに入れてください。（.storyboardの拡張子は含めない）", bundle: nil)
        self.window?.rootViewController = board.instantiateViewControllerWithIdentifier("スプラッシュスクリーンに利用する画面のStoryboard IDをここに入れてください。") as? UIViewController
        self.window?.makeKeyAndVisible()
    }
    
    private func setupRootViewController() {
        let board = UIStoryboard(name: "Storyboardのファイル名をここに入れてください。（.storyboardの拡張子は含めない）", bundle: nil)
        self.window?.rootViewController = board.instantiateViewControllerWithIdentifier("ログイン後に表示する画面のStoryboard IDをここに入れてください。") as? UIViewController
    }
